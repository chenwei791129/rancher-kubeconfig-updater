...
.DESCRIPTION
    ...
    The script body is wrapped in `Install-RancherKubeconfigUpdater` and invoked
    inside a try/catch so that, when this file is piped to `iex`, failure paths
    do not terminate the caller's PowerShell session via `exit`. The function
    uses `throw` for failures (caught by the outer wrapper) and `return` for
    early success exits. `$LASTEXITCODE` is set to 1 on failure for CI use.
...
function Install-RancherKubeconfigUpdater {
    [CmdletBinding()]
    param()
    ...
    if ($Version -eq 'latest') {
        ...
        try {
            ...
            # literal "latest" if we cannot resolve the redirect.
            try {
...
# Run the installer. The outer try/catch keeps `iex` callers' sessions alive
# when the installer throws on a failure path - it sets `$LASTEXITCODE` for
# scripted callers but does not call `exit`, which would close the host.
try {
    Install-RancherKubeconfigUpdater
    $global:LASTEXITCODE = 0
} catch {
    # Write-Err has already printed a user-facing message before each throw,
    # so the catch only needs to set the exit code. Re-printing the exception
    # would duplicate the error.
